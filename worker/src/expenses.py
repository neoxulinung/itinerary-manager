...
async def _refresh_display_name(env, user_id: str, current_name: str | None) -> str:
    # get_display_name falls back to the raw userId when the person hasn't friended the
    # bot yet. That fallback used to get stored as if it were their real name and then
    # stay wrong forever, even after they later friended the bot (found in real use).
    # Detect the stale case (stored name == the raw id) and retry live; if it resolves
    # now, fix every row it's stored in so this only ever has to self-heal once.
    ...
    ).bind(fresh, user_id, user_id).run()
    await env.DB.prepare(
...
async def add_expense(
    ...
    await env.DB.prepare(
        ...
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
    ).bind(expense_id, trip_id, payer_id, payer_display_name, amount, description, now, now).run()
    for p in participants:
        await env.DB.prepare(
            "INSERT INTO expense_splits (expense_id, line_user_id, display_name) VALUES (?, ?, ?)"
        ).bind(expense_id, p["line_user_id"], p["display_name"]).run()
...
async def update_expense(env, expense_id: str, amount: float, description: str, participants: list[dict]) -> None:
    ...
    ).bind(amount, description, now, expense_id).run()
    await env.DB.prepare("DELETE FROM expense_splits WHERE expense_id = ?").bind(expense_id).run()
    for p in participants:
        await env.DB.prepare(
            "INSERT INTO expense_splits (expense_id, line_user_id, display_name) VALUES (?, ?, ?)"
        ).bind(expense_id, p["line_user_id"], p["display_name"]).run()
...
async def list_expenses(env, trip_id: str) -> list[dict]:
    ...
    split_rows = await env.DB.prepare(
