...
async def start_poll(env, trip_id: str, user_id: str, topic: str) -> str:
    ...
    try:
        await env.db.query(
            ...
            "VALUES (?, ?, ?, 'active', ?, ?)",
            [poll_id, trip_id, topic, user_id, now],
        )
    except Exception:
...
async def get_options_with_votes(env, poll_id: str) -> list[dict]:
    options_result = await env.db.query(
        "SELECT id, text FROM poll_options WHERE poll_id = ? ORDER BY created_at", [poll_id]
    )
...
async def toggle_vote(env, option_id: str, user_id: str, display_name: str) -> bool:
    ...
    existing = await env.db.query(
        "SELECT 1 FROM poll_votes WHERE poll_option_id = ? AND line_user_id = ?", [option_id, user_id]
    )
    if existing.results:
        await env.db.query(
            "DELETE FROM poll_votes WHERE poll_option_id = ? AND line_user_id = ?", [option_id, user_id]
        )
        return False
...
async def end_poll(env, trip_id: str) -> str:
    ...
    now = int(time.time())
    await env.db.query("UPDATE polls SET status = 'ended', ended_at = ? WHERE id = ?", [now, poll["id"]])
